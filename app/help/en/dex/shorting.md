...
In order to increase your exposure to ESH and offer liquidity to BitAssets, such
...
The Esher network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 ESH as it is traded on external exchanges.
...
The Esher network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/ESH).
...
market rates up to the SQP and close the position. The remaining ESH of the
...
increase, an additional amount of ESH is locked as collateral, while reducing
...
particular bitAsset to hand it over to the Esher network. After that, the
